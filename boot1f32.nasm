...
start:
    ...
    add     eax, [gPartLBA]          ; brug BPB HiddenSectors (partition start LBA)
    mov     [gPartLBA], eax          ; gPartLBA bliver nu FAT-start LBA (som koden forventer senere)
...
; ----------------------------------------------------------------------------
; readLBA - læs sektorer med INT13h Extensions (AH=42h) via statisk DAP i gExtInfo
;
; Input:
;   AL  = antal sektorer (1..127 typisk)
;   ECX = sektor-offset relativt til partition (LBA32)
;   EDX = lineær destination-adresse (hvor data skal skrives)
;   [gBIOSDriveNumber] = BIOS drive (0x80 + disk)
;   [gPartLBA]         = partitionens start-LBA (LBA32)
;
; Output:
;   CF = 0 success
;        1 error (hopper til error: som hænger)
; ----------------------------------------------------------------------------
readLBA:
    pushad
    push    es

    ; --- Konverter lineær EDX -> ES:BX (positiv offset, BIOS-venlig) ---
    mov     bx, dx
    and     bh, 0x0F
    shr     edx, 4
    xor     dl, dl
    ...
    ; --- Byg DAP (16 bytes) i gExtInfo ---
    ; DAP layout:
    ; +0  BYTE  size (=16)
    ; +1  BYTE  reserved (=0)
    ; +2  WORD  sector count
    ; +4  WORD  buffer offset
    ; +6  WORD  buffer segment
    ; +8  DWORD LBA low
    ; +12 DWORD LBA high

    mov     byte [gExtInfo + 0], 16
    mov     byte [gExtInfo + 1], 0

    xor     ah, ah                      ; AX = sectors (AL), AH=0
    mov     word [gExtInfo + 2], ax

    mov     word [gExtInfo + 4], bx     ; buffer offset
    mov     word [gExtInfo + 6], es     ; buffer segment

    add     ecx, [gPartLBA]             ; absolut LBA = partition start + offset
    mov     dword [gExtInfo + 8],  ecx  ; LBA low
    mov     dword [gExtInfo + 12], 0    ; LBA high (vi bruger 32-bit LBA)

    ; --- INT 13h, AH=42h Extended Read ---
    mov     dl, [gBIOSDriveNumber]
    mov     si, gExtInfo                ; DS antages = 0 som i start:
    mov     ah, 0x42
    int     0x13
    jc      error
