//
// GrOVeR patch notes (BergDK)
//
// Modified from upstream OpenCorePkg / OpenDuetPkg (Acidanthera).
//
// Changes:
// 1) Boot target path changed to a deterministic EFI path used by GrOVeR.
// 2) Boot selection flow adjusted for BIOS/GRUB chainloading use-case.
//
// Rationale:
// GrOVeR relies on a GRUB-driven legacy chainload path where disk-signature
// gating and certain BDS selection behavior is not reliable across hardware.
//
// License:
// Upstream: BSD-3-Clause (see LICENSES/ and upstream LICENSE.txt)
//
